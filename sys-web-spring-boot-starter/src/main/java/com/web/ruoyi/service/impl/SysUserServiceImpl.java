...
//        return list.stream().map(SysPost::getPostName).collect(Collectors.joining(","));
    ...
    {
        Long userId = RuoyiStringUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        SysUser info = userMapper.checkUserNameUnique(user.getUsername());
        if (RuoyiStringUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue())
        {
    ...
    {
        if (RuoyiStringUtils.isNotNull(user.getUserId()) && user.isAdmin())
        {
//            throw new ServiceException("不允许操作超级管理员用户");
            throw ExceptionUtil.business(RuoyiErrorCodeEnums.REFUSE_MODIFY_ADMIN_USER);
        }
    }
...
//            if (RuoyiStringUtils.isEmpty(users))
//            {
...
//        if (RuoyiStringUtils.isNotEmpty(posts))
//        {
//            // 新增用户与岗位管理
...
//    }
    ...
    {
        if (RuoyiStringUtils.isNotEmpty(roleIds))
        {
            // 新增用户与角色管理
...
//        userPostMapper.deleteUserPost(userIds);
    ...
    {
//        if (RuoyiStringUtils.isNull(userList) || userList.size() == 0)
//        {
